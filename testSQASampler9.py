...
sampler = SQASampler()
response = sampler.sample_qubo(Q, num_sweeps=4000, num_reads=10)

# 復号
def decode_xy_from_bits(bits):
    xb = bits[:v_num]
    yb = bits[v_num+sv_num : v_num+sv_num+v_num]
    u = float(np.dot(w, xb))
    v = float(np.dot(w, yb))
    x_val = x0 + beta * u
    y_val = y0 + beta * v
    return xb.tolist(), yb.tolist(), x_val, y_val

def eval_himmelblau(x, y, a=11, b=7):
    return (x*x + y - a)**2 + (x + y*y - b)**2

# 補助変数一致
def aux_consistency(bits):
    # x 側
    r = v_num
    ok_x = True
    mis_x = 0
    for s in range(v_num):
        for t in range(s+1, v_num):
            if bits[s]*bits[t] != bits[r]:
                ok_x = False
                mis_x += 1
            r += 1
    # y 側
    r = v_num + sv_num + v_num
    ok_y = True
    mis_y = 0
    for s in range(v_num):
        for t in range(s+1, v_num):
            if bits[v_num+sv_num+s]*bits[v_num+sv_num+t] != bits[r]:
                ok_y = False
                mis_y += 1
            r += 1
    return ok_x, ok_y, mis_x, mis_y

print("=== Per-sample summary (energy, x_n/y_n bits, aux, decimal x/y, f(x,y)) ===")
rows = []
for i, (E, bits) in enumerate(zip(response.energies, response.states)):
    xb, yb, x_dec, y_dec = decode_xy_from_bits(bits)
    okx, oky, misx, misy = aux_consistency(bits)
    fval = eval_himmelblau(x_dec, y_dec, a, b)
    rows.append({
        "idx": i,
        "energy": float(E),
        "x_bits": "".join(str(int(b)) for b in xb),   # 例: 11010
        "y_bits": "".join(str(int(b)) for b in yb),
        "x": float(x_dec),
        "y": float(y_dec),
        "aux_ok_x": bool(okx),
        "aux_ok_y": bool(oky),
        ...
        "mis_y": int(misy),
        "f(x,y)": float(fval),
        # "all_bits": "".join(str(int(b)) for b in bits),  # ←全ビット(補助含む)も残したければコメント解除
    })
    print(f"[{i:02d}] E={E:.6f} | x={x_dec:.6f}, y={y_dec:.6f} | aux_ok(x,y)=({okx},{oky}) mis=({misx},{misy}) | f(x,y)={fval:.6f}")

if rows:
    ts = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = f"results_{ts}.csv"   # 実行ディレクトリに保存（絶対パスにしたい場合は適宜変更）
    ...
    print("[WARN] No samples to write.")
# | x_bits={xb} y_bits={yb} |